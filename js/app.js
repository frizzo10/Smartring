...
function buildWtSteps(){
  ...
  return[
    {title:'Welcome',text:`Hi ${name}. I am Dr. Sage, your personal health advisor. I watch your ring data every single day. My job is not to replace your doctor — it is to make sure that every time you see your doctor, you arrive prepared, informed, and with real data they can actually use. Let me show you around.`},
    {title:'What I monitor',text:`Your ring measures your heart rate, heart rate variability, blood pressure, blood oxygen, skin temperature, and sleep — continuously, every night. I analyze those patterns daily. When I notice something worth paying attention to, I tell you what it means, what to ask your doctor, and I document it in a report they can read in 60 seconds.`},
    {title:'I remember everything',text:`Here is something important, ${name}. I have a memory. Every time we talk, I remember what you tell me — your stress levels, what your doctor said last visit, what you have tried, what is working. Over time, I build a complete picture of your health that no doctor visit alone could capture. The longer you use me, the more useful I become.`},
    {title:'Your health today',text:`Here is where you stand today. Your readiness is ${t.readiness} out of 100. Blood pressure is ${t.bpSys} over ${t.bpDia}. HRV is ${t.hrv} milliseconds. Overall health grade: ${grade}. These numbers tell a story — and the longer you wear the ring, the clearer that story becomes.`},
    {title:'How we talk',text:`When I notice a pattern — a temperature spike, a blood pressure trend, a change in your sleep — I will flag it on your dashboard. Tap my card to start a voice conversation. I will ask what is going on in your life. We build a plan together. And I follow up to see if it worked. Every conversation makes me smarter about you.`},
    {title:'The doctor report',text:`This is the feature I am most proud of. Before every doctor visit, tap Dr. Report at the bottom of the screen. I generate a clean, clinical summary with your trends, my findings, and specific questions worth raising — tailored to whichever specialist you are seeing. Your doctor will actually read it. That moment — handing them a real report — is what this is all for.`},
    {title:'Weekly Check-in',text:`Every week I put together a summary for you. Tap Weekly Check-in at the bottom to see your weekly narrative, your daily recovery chart, and one specific thing worth bringing up at your next doctor visit. Think of it as your weekly check-in with me — I will tell you what I noticed and what I think it means.`},
...
function renderDoctorPrep(prep, el) {
  el.innerHTML =
    ...
    '<div style="display:flex;gap:10px;margin-bottom:12px;">' +
      '<button onclick="emailDoctorPrep()" style="flex:1;height:48px;border-radius:13px;background:var(--accent);color:#fff;border:none;font-size:15px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;">' +
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>' +
        'Email to myself' +
      '</button>' +
      '<button onclick="closeWeekly();openReportModal();" style="flex:1;height:48px;border-radius:13px;background:var(--fill);border:1px solid var(--hairline);color:var(--ink);font-size:15px;font-weight:500;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;">' +
...
function emailDoctorPrep() {
  const question = document.getElementById('prep-question')?.textContent?.replace(/^"|"$/g,'').trim();
  if (!question) return;

  // Get the stored prep data
  const topic = document.querySelector('[id="encounter-content"] p[style*="font-size:18px"]')?.textContent || 'Health update';
  const name = profile.name || 'Frank';
  const date = new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});

  const subject = encodeURIComponent('Reminder: what to tell my doctor — ' + topic);
  const body = encodeURIComponent(
    'Hi ' + name + ',\n\n' +
    'Here is your Doctor Prep reminder from myDrSage for your upcoming appointment.\n\n' +
    '----------------------\n' +
    'SAY THIS TO YOUR DOCTOR:\n' +
    '----------------------\n' +
    '"' + question + '"\n\n' +
    '----------------------\n' +
    'Generated by myDrSage - ' + date + '\n' +
    'myDrSage.com - Always watching. So your doctor does not have to start from scratch.\n\n' +
    'For the full clinical report with your 7-day biometrics, open myDrSage and tap Dr. Report.'
  );

  window.location.href = 'mailto:' + (profile.email||'') + '?subject=' + subject + '&body=' + body;
  showToast('📧 Email opened', 'Send it to yourself so you have it at the appointment.');
}
...
Style: plain English, 4–6 sentences, one follow-up question. Suggest bringing findings to their doctor when relevant.`;
  try{
    const res=await fetch('/.netlify/functions/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'qwen/qwen3.6-27b',max_tokens:1000,system:sys,messages:chatMessages})});
...
{"patientSummary":"2-3 sentence overview","keyFindings":[{"metric":"string","current":"string","previous":"string","trend":"improving|stable|worsening","note":"string"}],"concernsForDoctor":["string"],"recommendedDiscussion":["string"],"lifestyle":"brief lifestyle summary paragraph","disclaimer":"standard disclaimer"}`;
  try{
    const res=await fetch('/.netlify/functions/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'qwen/qwen3.6-27b',max_tokens:1500,messages:[{role:'user',content:prompt}]})});
...
async function handleResultUpload(e){
  ...
  reader.onload=async()=>{
    ...
    try{
      const res=await fetch('/.netlify/functions/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'qwen/qwen3.6-27b',max_tokens:1000,messages})});
...
async function saveDrResult(){
  ...
  try{
    const res=await fetch('/.netlify/functions/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'qwen/qwen3.6-27b',max_tokens:600,messages:[{role:'user',content:prompt}]})});
...
async function connectRing(btn) {
  ...
  // Wire debug panel BEFORE connect so discovery logs appear
